...
export default function MetaphorResultPage() {
  const router = useRouter();
  const { user } = useAuth();
  const { refreshTraits } = useTraits();
  const [result, setResult] = useState<MetaphorResult | null>(null);
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem('metaphor-result');
    if (!stored) {
      router.push('/dig/metaphor');
      return;
    }
    const data = JSON.parse(stored) as MetaphorResult;
    setResult(data);
    ...
    try {
      const saveResponse = await authenticatedFetch('/api/save-interview', {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: 'assistant', content: '自分を○○に例えると？', timestamp: new Date() }],
          interviewerId: 'female_01',
          mode: 'basic',
          sessionId: `metaphor-${Date.now()}`,
          status: 'completed',
        }),
      });
      ...
      console.error('Error saving metaphor traits:', error);
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="px-4 py-6">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="space-y-3">
          <button
            onClick={() => {
              sessionStorage.removeItem('metaphor-result');
              router.push('/mypage');
            }}
