...
export default function FavoritesResultPage() {
  const router = useRouter();
  const { user } = useAuth();
  const { refreshTraits } = useTraits();
  const [result, setResult] = useState<FavoritesResult | null>(null);
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem('favorites-result');
    if (!stored) {
      router.push('/dig/favorites');
      return;
    }
    const data = JSON.parse(stored) as FavoritesResult;
    setResult(data);
    ...
    try {
      const saveResponse = await authenticatedFetch('/api/save-interview', {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: 'assistant', content: 'あなたの好きな○○', timestamp: new Date() }],
          interviewerId: 'female_01',
          mode: 'basic',
          sessionId: `favorites-${Date.now()}`,
          status: 'completed',
        }),
      });
      ...
      console.error('Error saving favorites traits:', error);
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="px-4 py-6">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="space-y-3">
          <button
            onClick={() => {
              sessionStorage.removeItem('favorites-result');
              router.push('/mypage');
            }}
